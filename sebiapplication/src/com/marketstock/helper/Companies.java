...
import com.marketstock.sebiapplication.priceService;
...
public class Companies {
	...
	public static void updateData(String companyName) {
		...
		cursor = MainActivity.db.getReadableDatabase()
				.rawQuery(
						"SELECT * FROM companydata where company='"
								+ companyName + "'", null);
		cursor.moveToFirst();
		...
		if (!priceService.marketOpen) {
			PriceList.put(companyName, prevPriceList.get(companyName));
		}
	...
	public static void updateData52(String companyName) {
		...
		for (; !cursor.isBeforeFirst();) {
			low = Math
					.min(low, (float) cursor.getDouble(cursor
							.getColumnIndex("lowPrice")));
			high = Math.max(high, (float) cursor.getDouble(cursor
					.getColumnIndex("highPrice")));
