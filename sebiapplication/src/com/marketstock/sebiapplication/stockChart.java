...
public class stockChart extends SherlockFragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		final View rootView = inflater.inflate(R.layout.stock_chart, container,
				false);
		// GraphViewSeriesStyle seriesStyle = new GraphViewSeriesStyle();

		// GraphViewSeries series = new GraphViewSeries("Infosys", seriesStyle,
		// new GraphViewData[] {});
		final LineGraphView graphView = new LineGraphView(this.getActivity(),
				"Quote");
		...
		Cursor cursor = MainActivity.db.getReadableDatabase().rawQuery(
				"SELECT id,date,closePrice FROM infosys order by date DESC",
				null);
		cursor.moveToFirst();
		...
		graphView.setCustomLabelFormatter(new CustomLabelFormatter() {
			...
			public String formatLabel(double value, boolean isValueX) {
				if (isValueX) {
					...
					// return
					// dateFormat.format(indexToDate.get((int)value)).toString();
					return ((Date) dateMarker.get((int) value)).toString();
					// dateFormat.format(new Date((long)(value*(1000 * 60 * 60 *
					// 24))+startdate.getTime())).toString();
		...
		while (cursor.isAfterLast() == false) {
			...
			// Stock stock=new Stock(new
			// Date(i++),openPrice,closePrice,highPrice,lowPrice,volume);
		...
		final SeekBar seekBarSize = (SeekBar) rootView
				.findViewById(R.id.ChartSeekBarSize);
		final TextView startText = (TextView) rootView
				.findViewById(R.id.ChartStartSeekValue);
		final TextView sizeText = (TextView) rootView
				.findViewById(R.id.ChartSizeSeekValue);
		...
		seekBarSize.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
			...
			public void onProgressChanged(SeekBar seekBar, int progress,
				...
				graphView.setViewPort(
						Integer.parseInt((String) startText.getText()),
						progress);
